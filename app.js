...
function applyCurrentTakeoffLabel() {
  ...
  // v0.222: la altura se renderiza en letra mas pequena, no negrita y entre
  // parentesis, sin el punto separador ("Cenes <span>(81 m)</span>").
...
const SHARE_STR = {
  es: { title: "Parte de viento", best: "Mejor ventana",
  ...
  ca: { title: "Butlletí de vent", best: "Millor finestra",
...
// v0.241: captura fiel del panel de estado (#statusCard) con html2canvas, tal y
// como se ve en la app. Incluye brujula, anemometros, lecturas y el grafico de
...
async function shareCurrentForecast() {
  ...
  try {
    const canvas = await captureStatusCard();
    if (canvas) blob = await new Promise((res) => canvas.toBlob(res, "image/png"));
  } catch (e) { console.warn("[share capture]", e); }
